...
function initIntroAnim() {
	...
	jQuery('.intro-page').each(function() {
		...
		setTimeout(function() {
			page.addClass('anim-completed');

			setTimeout(function() {
				page.addClass(resetClass);
			}, 1000);
		}, animDelay);
	});
}
...
function initFancybox() {
	jQuery('a.light-box, [data-fancybox]').fancybox({
		parentEl: 'body',
		touch: false,
		autoFocus: false,
	});
...
function initAnchors() {
	new SmoothScroll({
		anchorLinks: 'a.smoth-scroll[href^="#"]',
		extraOffset: 0,
			...
			on: function() {
				jQuery('.js-open-close').openClose({
					...
					onInit: function(self) {
						self.slider.find('.link-list a').on('click', function() {
							self.hideSlide();
						})
					}
				});
...
function initStickyScrollBlock() {
	ResponsiveHelper.addRange({
		'768..': {
			on: function() {
				jQuery('.anchor-nav').stickyScrollBlock({
					activeClass: 'fixed-position',
					container: '.content-wrapper',
					positionType: 'fixed',
					customParent: '#sidebar',
					extraTop: function() {
						var totalHeight = window.innerHeight / 2 - jQuery('.anchor-nav').outerHeight() / 2;
						return totalHeight;
					}
				});
			},
			off: function() {
				jQuery('.anchor-nav').stickyScrollBlock('destroy');
			}
		}
	});
}
...
function initShowMore() {
	...
	jQuery('.post-block:has(.post-holder)').each(function(i) {
		var container = jQuery(this);
		var holder = container.find('.post-holder');
		var items = holder.find('.post');
		var modalOpener = holder.find('.read-more.light-box');
		var title = container.find('h6');
		var modalID = 'more-posts' + i;
		var modalOpenerTemplate = '<a href="#' + modalID + '" class="read-more green light-box">SEE MORE RELATED ARTICLES</a>';

		var template =  '<div class="popup-holder">' +
						'<div id="' + modalID + '" class="lightbox-content">' +
						'<div class="post-block">' +
						'<h6>' + title.text() + '</h6>' +
						'<div class="post-holder"></div>' +
						'</div>' +
						'<a data-fancybox-close href="#" class="close"><span class="sr-only">Close</span></a>' +
						'</div>' +
						'</div>';


		if (modalOpener.length) {
			modalOpener.attr('href', '#' + modalID);
		} else {
			jQuery(modalOpenerTemplate).insertAfter(holder);
		}
...
function initOpenCloseBlock() {
	...
	jQuery('.open-close-block').each(function() {
		...
		slides.each(function() {
			...
			link.on('click', function(e) {
				...
				if(holder.css('flex-direction') == 'column') {
					clearTimeout(timer);
		...
		function setActive(item) {
			if(activeItem.length) {
				...
				if(activeItem.data('video')) {
					activeItem.data('video').pause();
				}
			}
			
			activeItem = item;
			item.addClass(activeClass);
			item.data('video').play();
		}
	});
}
...
;(function($, $win) {
	...
	var StickyScrollBlockPrototype = {
		...
		destroy: function() {
			...
			if (this.options.customParent) {
				jQuery(this.options.customParent).removeClass('has-sticky');
			}
		...
		fixed: {
			scrollHandler: function() {
				...
				if (isActiveSticky) {
					...
					if (this.options.customParent) {
						jQuery(this.options.customParent).addClass('has-sticky');
					}
				} else {
					this.stickyOff();
					if (this.options.customParent) {
		...
		refreshActiveClass: function() {
			...
			$.each(this.anchorData, function(index) {
				...
				if (scrollTop >= containerHeight - viewPortHeight) {
					...
					if (!foundFlag && (scrollTop >= data.block.offset().top - Math.floor(win.height() / 4) || reverseIndex === 0)) {
						foundFlag = true;
						self.toggleActiveClass(anchorElement, data.block, true);
					} else {
						self.toggleActiveClass(anchorElement, data.block, false);
